    ...
    >
      ...
      >
        <div className="flex flex-col h-full bg-white">
          ...
          <div className="flex-grow flex flex-col items-center justify-center space-y-8 py-10 overflow-y-auto">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.href}
                ...
                className={`text-3xl font-black transition-colors ${
                  pathname === link.href ? "text-primary scale-110" : "text-zinc-400 hover:text-zinc-900"
                }`}
              >
                {link.name}
              </Link>
            ))}
            <div className="pt-8 space-y-4 w-full px-12">
              <Link
                href="tel:9471967853"
                className="w-full flex items-center justify-center px-8 py-5 bg-primary text-white text-lg font-bold rounded-2xl shadow-xl shadow-primary/20"
              >
                <Phone className="w-5 h-5 mr-3" />
                Call Now
              </Link>
              <p className="text-center text-zinc-400 text-sm font-medium uppercase tracking-widest">
                Serving Arwal Since 2014
              </p>
            </div>
          </div>
        </div>
